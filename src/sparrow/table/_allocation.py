...
def bin_counts(
    ...
    sdata
        The SpatialData object.
    table_layer
        The table layer holding the counts. E.g. obtained using `sparrow.io.visium_hd`.
    ...
    labels_layer
        The labels layer (e.g., segmentation mask, or a grid generated by `sparrow.im.add_grid_labels_layer`) in `sdata` used to bin barcodes (as specified via `table_layer`) into cells or regions.
    output_layer
    ...
    # sdata[table_layer].obsm[_SPATIAL] contains the positions of the barcodes if visium reader is used 'sparrow.io.visium_hd'
    name_x = "x"
    name_y = "y"
