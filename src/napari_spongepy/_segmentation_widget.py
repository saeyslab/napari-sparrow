...
def _segmentation_worker(
    ...
    # We delay importing the large cellpose module to when it is really
    # needed over here. This makes the segmentation widget appear almost
    # instantaneously when it is opened in napari, otherwise it would only
    # appear after the cellpose package is imported, which takes several
    # seconds.
...
def segmentation_widget(
    ...
    worker = _segmentation_worker(
        image.data, use_gpu, min_size, flow_threshold, diameter, mask_threshold
    )
