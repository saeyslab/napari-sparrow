...
# CHECKME: is this indeed the pixel size of the Resolve microscope images, in microns?
RESOLVE_PIXEL_SIZE = 0.138
...
@magic_factory(
    ...
    density_radius_micrometer={"min": 0.1, "max": 10.0},
)
def transcripts_widget(
    ...
    selected_genes = genes_to_plot.replace(" ", "").split(
        ","
    )  # remove whitespace and split, e.g. 'Acta2, Xcr1' -> ['Acta2', 'Xcr1']
    ...
    coords = selected_df.iloc[
        :, [1, 0]
    ]  # change the order from x,y to y,x for use in napari
    ...
    if density_plot:
        density_radius_pixels = math.ceil(
            density_radius_micrometer / RESOLVE_PIXEL_SIZE
        )
