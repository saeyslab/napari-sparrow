...
def plot_shapes(adata,column=None,cmap='magma',alpha=0.5,crd=None):
    ...
    if column is not None:
        ...
        adata.obsm['polygons'].plot(ax=ax,column=adata.obs[column],edgecolor='white',linewidth=adata.obsm['polygons'].linewidth,alpha=alpha,legend=True,cmap=cmap)
    else: 
        fig,ax =plt.subplots(1,1,figsize=(20,20))
        ax.imshow(adata.uns['spatial']['melanoma']['images']['hires'],cmap='gray',)
...
def clustercleanliness(adata,I,celltypes,crop_coord=[0,2000,0,2000],liver=False): 
    #The coloring doesn't work yet for non-liver samples, but is easily adaptable, by just not defining a colormap anywhere 
    ...
    if liver==True:
        ...
        Stacked_Norm.plot(kind='bar',stacked=True,ax=f.gca())   
    #ax.axis('off')
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.spines['bottom'].set_visible(False)
